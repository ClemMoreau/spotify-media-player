...
const PlayBackControls = ({player, is_paused} : PlayBackControlsProps) => {
    ...
    const handleRepeat = async () => {
        ...
        await sdk.player.setRepeatMode(reportMode as 'track' | 'context' | 'off')
        setRepeat(reportMode as 'track' | 'context' | 'off');
    }

    return ( 
        player ? <div className={css.btnSpotify}>
            ...
            <div className={css.btnSpotifyNext} onClick={async () => {await  player.nextTrack(); await player.pause() }} >
                <CgPlayTrackNext size={32} color={"#b3b2b2"} />
            </div>

            <div className={css.btnSpotifyLoop} onClick={handleRepeat} >
                <SlLoop size={16} color={isRepeat === "off" ? "#b3b2b2" : isRepeat === "context" ? "#FFFFFF" : "#1DB954"} />
            </div>
        </div> 
